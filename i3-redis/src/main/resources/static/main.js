 ...
 * Clear and reload the values in data table.
 */
function studentRefreshTable() {
    $.get('/student/values', function(data) {
        var attr,
        studentMainTable = $('#studentMainTable tbody');
        studentMainTable.empty();
        for (attr in data) {
            if (data.hasOwnProperty(attr)) {
            	studentMainTable.append(row(attr, data[attr]));
            }
        }
    });
}
